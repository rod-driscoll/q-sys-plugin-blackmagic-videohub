...
elseif(CurrentPage == 'Device') then 
  ...
  local UI_outputObjects = {
    ...
    Init = function(self)
      ...
      for o=0, self.NumButtons-1 do
        ...
        name_['Position']={ 
          UI_crosspoints.Outputs[0][0].Position[1], -- horiz always the same [output:0][input:0][horiz]
        ...
        btn_['Position']={ 
          ...
          name_.Position[2] } -- vert moves down to line up with the crosspoint buttons
        ...
        if o==1 then
          ...
          lbl_['Position']= {
            ...
            if o==0 then self.GroupBox.Size[1] = self.GroupBox.Size[1] + lbl_.Size[1] + self.Padding[1] end
          table.insert(self.Labels, lbl_)
        end

        -- left column of labels (numbers only)
        local num_ = helper.Copy(UI_crosspoints.Label)
        num_.Size = { 18, btn_.Size[2] }
        num_["Text"] = tostring(o)
        num_['Position']= {
          btn_.Position[1] + btn_.Size[1] + self.Padding[1], -- horiz always the same
          btn_.Position[2] } -- vert moves down to line up with the crosspoint buttons
        if o==0 then self.GroupBox.Size[1] = self.GroupBox.Size[1] + num_.Size[1] + self.Padding[1] end
  ...
  local UI_inputObjects = {
    ...
    Init = function(self)
      ...
      self.GroupBox.Size = {
        ...
        self.Padding[2]  + UI_crosspoints.Label.Size[2]+ self.Padding[2] } -- vert, increase as objects added   
      self.GroupBox.Position = self.Position
      self.GroupBox.Text="Inputs"

      for i=0, self.NumButtons-1 do
        -- top row of labels (numbers only)
        local lbl_ = helper.Copy(UI_crosspoints.Label)
        ...
        name_['Position']={ 
          ...
          lbl_.Position[2] + lbl_.Size[2] + self.Padding[2] } -- vert always the same
  ...
  -- add post fade objects (source list)
  local UI_postFadeObjects = {
    ...
    Init = function(self)
      ...
      self.GroupBox.Position = {
        ...
        UI_crosspoints.GroupBox.Position[2] }
